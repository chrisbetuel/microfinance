...
This is a direct port of the frontend's src/lib/loanMath.ts, kept in lockstep
with it deliberately: the backend must compute the same numbers the frontend
prototype does, and must never trust a client-submitted schedule or allocation.
"""
...
def classify_arrears_band(days_late: int) -> str:
    if days_late <= 0:
        return "current"
    if days_late <= 7:
        return "1-7"
...
def allocate_payment(product, schedule: list[ScheduleRow], amount: float) -> AllocationResult:
    ...
    # `schedule` may be ScheduleRow copies or ORM instalments (whose Numeric
